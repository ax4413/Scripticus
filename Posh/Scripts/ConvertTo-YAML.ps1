    ...
    PROCESS {
        ...
        try {
            ...
            switch  ($Type)
            {
                ...
                'xml'             {   # just use a 'newlines-preserved' string
                                      "|`r`n"+($inputObject.OuterXMl.Split("`r`n") | foreach{"$padding$_`r`n"})  
                                  } 
                'DateTime'        {   # s=SortableDateTimePattern (based on ISO 8601) using local time
