...
Function New-SymLink {
    <#
        ...
        .EXAMPLE
         ...
         .EXAMPLE
            New-SymLink -SymName "C:\users\syeadon.ICENET\Documents\WindowsPowerShell\Modules" -Path "D:\Code\Infrastructure Build Scripts\Modules" -Directory
            New-SymLink -SymName "C:\users\syeadon.ICENET\Documents\WindowsPowerShell\Scripts" -Path "D:\Code\Icenet 3\Trunk\src\Utilities\Powershell\Scripts" -Directory
    #>
    [cmdletbinding(
...
#remove any quotes if this has been passed as a string
$PathToModules =$PathToModules -replace "'"
$PathToModules =$PathToModules -replace "`""
...
if(-not (Test-Path $userFolder -PathType Container)){
    Write-Host "ERROR"
    Write-Host "A user profile does not exist for $userName" -ForegroundColor Red
    Write-Host "  $userFolder" -ForegroundColor Red
    Write-Host ""
    Write-Host "Your current user profile is" -ForegroundColor Red
    write-host "  $env:USERPROFILE" -ForegroundColor Red
    exit
}


if(-not (Test-Path $PathToModules -PathType Container)){
    Write-Host "ERROR"
    Write-Host "The path to the PoSH modules does not exist. Please check out the modules.." -ForegroundColor Red
    Write-Host "  $PathToModules" -ForegroundColor Red
    exit
}


$moduleFolder = Join-Path $userFolder -ChildPath "Documents\WindowsPowerShell\Modules"


New-SymLink -SymName $moduleFolder -Path $PathToModules -Directory -ErrorAction Stop
